...
; ----------------------------------------
  ...
  ; Jump based on how many times I can decrement X before it's zero.
...
FillIt:
  ...
  ; Make a level cache source pointer
  lda #0
  sta 0 ; High
  sta ScrollCacheUpdateDestination+1
  lda CacheUpdateChunk
  asl ; 2 blocks per chunk
  asl
  rol 0
  asl
  rol 0
  ...
  pha
  lda 0
  sta ScrollCacheUpdateSource+1 ; High
  pla
  sta ScrollCacheUpdateSource   ; Low

  ; Make a level cache destination pointer
  lda CacheUpdateChunk
  and #15
  ora #%00110000 ; Makes sure the resulting pointer is $6xx or $7ff
  .repeat 5
    asl
    rol ScrollCacheUpdateDestination+1
  .endrep
  sta ScrollCacheUpdateDestination

  ; Make a pointer to the level data that
  ; the blocks will be taken from.
  ldx #0
  stx ScrollLevelPointer+1
...
; Runs at scroll stage 1
.proc UpdateLevelCache
  ldy #31
  lda ScrollCacheUpdateDestination+0
  sta 0
  lda ScrollCacheUpdateDestination+1
  sta 1
: lda ThirtyUpdateTile,y
  sta (0),y
  dey
  bpl :-
  rts
